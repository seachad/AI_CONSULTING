...
- Comunidad (D80): la página `SEVEN-G/herramientas/comunidad/` está publicada en **solo lectura** hasta que el autor cree el repositorio público `seachad/seven-g-feedback`, un token de grano fino (solo ese repositorio, «Issues: Read and write») y el Cloudflare Worker con `worker/worker.js`, y ponga su URL en `endpoint` (bloque `config` de `index.html`); pasos en el README de esa carpeta. El token nunca se escribe en el repositorio. Pendiente de decidir: `umbralVotos` (hoy no se muestra ningún número). Si cambia `worker.js`, pasar `_pruebas/prueba_worker.html` y volver a pegarlo en Cloudflare.
