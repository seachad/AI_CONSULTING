...
- **Adaptar y regenerar**: editar el Markdown y ejecutar `pwsh -File SEVEN-G/build/build.ps1` (PowerShell 7 y Microsoft Edge); el generador del panel del consejo (T17) necesita Python 3.11 con `uv`.
- **Una instalación por cliente**: un origen web distinto por cliente (subdominio, dominio o puerto), un registro construido con los datos del cliente y su panel generado con T17.
