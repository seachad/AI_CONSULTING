...
function Nuevo-Indice([string]$lang) {
  ...
  if ($en) {
    ...
    [void]$sb.AppendLine("> **Aviso legal y exención de responsabilidad.** Estos contenidos y herramientas se ofrecen «tal cual» y con fines exclusivamente informativos. No constituyen asesoramiento jurídico, regulatorio, financiero ni profesional, ni garantizan el cumplimiento de ninguna norma. Cada organización que los use es la única responsable de identificar la normativa que le aplica, verificar su vigencia y certificar su propio cumplimiento regulatorio. Esta metodología es una ayuda genérica y gratuita, compartida con la comunidad para que nadie tenga que empezar desde cero; cada persona u organización puede y debe adaptarla a su propio uso. No debe entenderse que sus partes jurídicamente sensibles hayan sido revisadas por una asesoría jurídica: esas revisiones, para cada empresa o sector, son responsabilidad última de la empresa, el consultor o la organización que la use. Aunque se procura mantenerla al día, alguna norma puede haber cambiado sin que se recoja aquí. En la máxima medida permitida por la ley, el autor no asume responsabilidad alguna por los efectos de su aplicación en ninguna organización ni por su aplicabilidad completa. La metodología no otorga certificación de ningún tipo. El autor no asume responsabilidad alguna por su uso.`n")
  }
  $i = 0
...
# curso completo descargable en PPT y PDF (D85): el PDF necesita el HTML de los módulos ya generado. Se regenera después
# la guía del curso (M00) para que su zona de descargas enlace ya el PPT y el PDF recién creados (build.ps1 los comprueba
...
if ($Metodologias -contains 'SEVEN-G' -and -not $env:SEVENG_BUILD_CURSO_PASE2) {
  ...
  if (-not $SinPdf) {
    ...
    try { & $PSCommandPath -Metodologias SEVEN-G -Filter 'M00_SEVEN-G_Curso_Guia_del_curso.md' -Idiomas $Idiomas }
