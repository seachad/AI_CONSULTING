...
<!-- esencial: condicional | Trigger: the initiative changes people's work, releases capacity or is involved in decisions about people. Limits that are never relaxed: maximum autonomy A1 in selection and appraisal, and A0 in discipline and dismissal; information to the workforce and its representatives when the law requires it. -->
...
```mermaid
flowchart LR
  M["Measured released capacity"] --> Q{"Decision recorded?"}
  Q -->|"No"| D5["PER-D5 No decision: reported separately and not counted"]
  Q -->|"Yes"| DEC["Destination decided"]
  DEC --> D1["PER-D1 Materialise: lower actual cost"]
  DEC --> D2["PER-D2 Reassign: activity with measured outcome"]
  DEC --> D3["PER-D3 Absorb growth"]
  DEC --> D4["PER-D4 Reinvest in quality or service"]
  D1 --> V["Value with validation status"]
  D2 --> V
  D3 --> V
  D4 --> NS["Value not additive unless formula"]
