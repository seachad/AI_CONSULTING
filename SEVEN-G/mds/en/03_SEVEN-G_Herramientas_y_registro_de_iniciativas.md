...
The model is the foundation of all the tools. Its full specification (fields, types, closed lists and validation rules) is the JSON schema of the register, `herramientas/T01_registro_iniciativas/esquema_registro.schema.json` (version 0.7), published with tool T01; each version extends it with optional fields only, so a file from an earlier version remains valid.
...
| **Maturity assessment** | Summary of each maturity assessment (EM-YYYY-MM; document 11): cut-off date, mode, questionnaire version, overall level, weighted average, cap by D1 or D6 and level, progress and blocking criteria of each dimension. Since schema 0.7, also the summary of the NIST profiles derived from the questionnaire (AI RMF and CSF 2.0: minimum level, subcategories with a level, with a target and with a gap, by function and, for the CSF, by area; 11 §7.5). T15 writes it (answers and evidence stay in T15) and the board dashboard shows it; schemas 0.6 and 0.7. | Company |
