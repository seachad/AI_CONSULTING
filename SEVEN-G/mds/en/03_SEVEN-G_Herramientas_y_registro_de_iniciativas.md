...
|---|---|---|---|
| **T11** (with T13) | Initiative (name, ambition, intensity, phase, spent and pending investment) and the expected amounts recorded. | The expected amounts of the use case (efficiencies, return, released capacity, recurring cost and investment), as expected with source T11, plus an edit event. | Parameters (horizon, rate, hourly cost), broken-down value lines, ramp-up, stop criteria, stages, costs per use case and reconciliation. |
