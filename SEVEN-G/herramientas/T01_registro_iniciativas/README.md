...
- **Excel del caso: matriz y planes** (botón de la pestaña Riesgos de la ficha y «Excel de un caso…» en la vista Riesgos; D106). Genera en el navegador, sin dependencias, un libro `.xlsx` con siete hojas: **Portada** (identificación del caso, responsables, fecha y hora de generación, esquema, resumen y aviso legal), **Matriz** (5 × 5 residual e inherente con el número de riesgos por celda, resumen por nivel y por categoría), **Registro** (todos los riesgos con los campos de P12 y 33 §8.1, niveles calculados y observaciones), **Plan de mitigación** (P13 §4: por cada riesgo abierto, acciones propuestas, tipo de control y qué reduce, y columnas a completar: responsable, fecha límite, coste, evidencia de eficacia y estado), **Plan de contingencia** (P13 §6: escenario, disparador y acciones inmediatas propuestos, uso del plan de reversión, quién lo activa, comunicación y notificaciones; obligatorio en Alto, Crítico e impacto 5, y marca cuando falta), **Riesgos a considerar** (los riesgos tipo del documento 33 §9 que aplican al perfil de la iniciativa —tecnología, exposición, autonomía, clasificación regulatoria, ambición y terceros— y no tienen un riesgo registrado con su código, con su mitigación y contingencia propuestas, la columna «Cuándo revisar» —«Ahora» si el documento 33 sitúa el riesgo en la fase actual del caso o antes, «Más adelante» en caso contrario, ordenados así— y columnas para incluir o descartar con motivo, 33 §8.2) y **Escalas**. Colores: blanco, datos del registro; azul, propuesta del catálogo, a validar; amarillo, a completar. El fichero se llama `SEVEN-G_T06_<código>_<AAAAMMDD-HHMMSS>.xlsx`. Las propuestas salen de `catalogo_riesgos.json` (reglas de aplicabilidad, tipo de control y textos de mitigación y contingencia por código RT; el título, la descripción, las causas y los controles tipo se leen del documento 33 §9 al generar `registro.html`): son un punto de partida para P13, no sustituyen la decisión del órgano competente y nada se estima. Un riesgo sin código RT, o con uno que no está en el catálogo, sale sin propuesta.
...
- **Resultados de otras herramientas** (D100): el mismo selector admite los ficheros que exportan T11 («resultado del caso», con `valores_t01`) y T15 («resumen para T01», con `madurez_t01`), y los incorpora al registro actual sin sustituirlo: los importes esperados del caso se añaden a la iniciativa (como esperados, con fuente T11 y un evento de edición) y el resumen de cada diagnóstico de madurez se une por identificador en `madurez[]`. Servidas desde el sitio, T11 y T15 pueden escribirlo directamente en el registro guardado en el navegador, sin ficheros.
- **Otras herramientas que leen este registro**: la vista Datos enlaza T11, T14 y T15 con el registro cargado (`?desde=t01`; el enlace guarda antes el registro en el navegador para que la otra herramienta lo lea), igual que la ficha (T11 con la iniciativa) y la vista Consejo (T14 y T15). El registro es la fuente de verdad: esas herramientas muestran de dónde viene cada dato y lo corregido a mano en ellas no se pisa. El mapa campo a campo está en `../mapa_datos.json` (documento 03 §4.1).
