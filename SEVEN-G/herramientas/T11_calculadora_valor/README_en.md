...
**With the T01 register in the same browser** (D100): the initiative register is the source of truth and, served from the site, the tools share local storage, so "New from a T01 register…" directly offers the initiatives of the saved register (no files) and the link "Calculate the value hypothesis with this initiative (T11)" in the Value tab of the T01 record opens T11 with the use case of that initiative created (or the existing one). A freshly opened T11 with a real register takes its company and removes the example use cases. The use case remembers its initiative (`iniciativa_t01`) and **"Save the expected amounts of this use case into the T01 register"** writes into the initiative the expected amounts of the use case (efficiencies, return, released capacity, recurring cost and investment), always as expected and with source T11, plus an edit event; T01 shows them in the Value tab. If the register is not in the browser, "Load a T01 file…" and the use case result export (`valores_t01`, which T01 incorporates with "Validate and merge") do the same through files. **"Refresh this use case from the register"** (D102) reads the initiative again: it renews name, ambition, intensity and phase, the amounts still "from T01" (each imported line remembers its source amount) and adds the new expected amounts; a line whose amount was changed by hand or that was broken down into units × unit value is kept, and one no longer in the register is flagged. When T11 is opened from the T01 record with an existing use case, it refreshes by itself.
