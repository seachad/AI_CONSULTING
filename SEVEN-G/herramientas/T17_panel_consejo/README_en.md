...
| `t01_a_panel.js` | The same mapping in JavaScript (D103): embedded in the generated dashboards (they regenerate themselves in the browser) and in the T01 register. |
...
| `config_panel.json` | General configuration of the dashboard: `navegacion` (no “Todo” page, initial page, cards expanded on entry, filters in a modal dialog with the applied query shown as pills and, for a dashboard hosted inside a site, `tema_sitio` —the `localStorage` key holding the site's general theme, which the dashboard follows and updates— and `sitio` —links back to the site in the side menu, with paths relative to the output folder— and `codigos` —path to the site's code index, `codigos.js`: with it, the codes written in the full dashboard, the mobile one and the recommendations register (document 40, T01, P12, G3.05…) become links to where they are explained, and their bar offers "Document finder" and "Cited here" (D99, D105), with no visit measurement; a dashboard generated outside the site removes those keys—), `umbrales_kpi` and `ciclo_vida` (funnel, exits, day limits and the stage of each SEVEN-G phase). Starting values, to be calibrated by each organisation. `datos_t01`: path of the company copy's T01 register that the dashboard converts in the browser (D103). `ayuda_tarjetas` (D110): with `true`, each indicator and card of the full dashboard, and each section of the mobile one, carries a “?” next to its title that opens, **in Spanish or English** (selector in the window itself; the choice is remembered in the browser), what it shows, how to read it, **what each of its columns or figures means**, why it matters and the SEVEN-G document that explains it; in addition, **every table header and box label** in the dashboard, including the records, shows what it means as a tooltip. The texts live in `motor/ayuda_textos.py` (cards and column dictionary, ES/EN) and are used by `motor/ayuda_tarjetas.py`; the top-level `ayuda_tarjetas` key (outside `navegacion`) can replace a card's title or texts (`{"es": {…}, "en": {…}}`), or remove its help with `false`. Without the key there is no “?” and no tooltips. A new dashboard column or card needs its entry in `ayuda_textos.py` in both languages (`python motor/ayuda_tarjetas.py` checks it, as does `verificar_coherencia.ps1`, section 23). The rest of the dashboard is in Spanish. |
| `motor/` | The dashboard engine, **version 8** (funnel and lifecycle, configurable thresholds): `build_dashboard.py` (full dashboard), `panel_movil.py`, `panel_core.py` (shared JavaScript core), `ayuda_tarjetas.py` and `ayuda_textos.py` (“?” button, ES/EN texts of each card and explanation of each column, D110), `economia.py`, `glosario.py`, `snapshot.py`, `ESQUEMA.md` (JSON schema) and `demo_lib.py` (recommendations log template). Copy maintained in AI_CONSULTING; origin: `AI_en_el_consejo/motor` (MIT, same author). |
...
## Without Python: the dashboard regenerates itself in the browser (D103)
