...
function setCard(id, title, note, insight, body){
  document.getElementById(id).innerHTML = limpiaRefs(`<summary><h3>${title}</h3><div class="ins">${insight}</div></summary><div class="body">${note?`<div class="note">${note}</div>`:""}${body}</div>`);
}
