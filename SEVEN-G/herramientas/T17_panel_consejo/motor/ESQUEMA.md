...
| `valor_imputado_multiplicador` | Opcional; si falta o es `0`, desactivado. Con un valor *m* > 0, un caso con coste recurrente y sin valor (ni eficiencias que cuenten ni retorno) recibe un **valor imputado** = *m* × coste recurrente anual (igual en el potencial con el coste recurrente potencial). Se muestra aparte y marcado con ⚠ en tarjetas, tabla, fichas, KPIs y panel móvil: no es un dato de la organización. Texto opcional en `textos.aviso_imputado` (admite `{mult}`). |
...
| `textos` | Textos de contexto opcionales: `aviso_previo`, `aviso_valor`, `pie`, `movimientos_vacio`, `agilidad_sin_fechas`, `backlog_sin_dato`, `adopcion_sin_telemetria`, `ref_guardarrailes`, `kpi_retorno_detalle`, `kpi_eficiencias_detalle`, `nota_concentracion`, `sin_que_es`, y etiquetas `ret_<concepto>` y `ef_<concepto>`. |
...
| `compania`, `unidad`, `estado` | `estado`: `En uso`, `En desarrollo`, `POC`, `Desenganchado`. |
...
Cifras derivadas, iguales en `economia.py` y en el panel: **neto anual** = eficiencias materializadas + retorno + valor imputado (si está activado) − coste recurrente; **neto potencial**, igual con los potenciales; **neto adicional por euro** = (neto potencial − neto actual) / inversión adicional; **payback** = construcción / neto anual.
