...
| `navegacion` | Opcional. Menú del panel completo: `pagina_todo` (`false` suprime la página «Todo»), `pagina_inicial` (`cartera`, `embudo`, `historico`, `riesgo`, `inventario` o `glosario`; por defecto «Todo») y `desplegar_todo` (`true`: al entrar en una página, todas sus tarjetas se muestran desplegadas). |
