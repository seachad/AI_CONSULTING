...
4. Complete los datos que faltan. Lo que se deja vacío cuenta como **sin dato**: puntúa 0, nunca se estima y reduce la cobertura. Con menos de 6 de 8 señales con dato, el perfil es **provisional**.
5. **Cómo leer las señales** (D108): sobre la tabla de señales, «Cómo leer esta tabla» explica cada columna (señal, valor medido, puntuación de 0 a 3, lectura y tendencia) y enlaza el documento 12 §4.4. En cada fila, «Cómo se calcula» muestra qué mide la señal, su fórmula, la cuenta con los datos de este cálculo, los cuatro tramos con los umbrales de la versión usada (resaltado el que aplica), por qué ha salido esa puntuación y el enlace a la señal en el documento 12. Los importes, horas y recuentos se muestran y se escriben con separador de miles (ES 2.650.000; EN 2,650,000; también se aceptan sin separador).
6. Anote en «Acciones» el responsable y el plazo de cada condición de «Qué movería el perfil» y el plan de datos (documento 12, sección 10). «Imprimir informe» da la hoja para el consejo.
7. «Datos» → «Exportar JSON de T14» guarda umbrales y cálculos, con sus resultados, para conservarlos o compartirlos.
